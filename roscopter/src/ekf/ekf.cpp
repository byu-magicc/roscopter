...
void EKF::load(const std::string &filename, const std::string &param_dictionary)
{
  // Constant Parameters
  get_yaml_eigen("p_b2g", filename, p_b2g_, param_dictionary);
  get_yaml_diag("Qx", filename, Qx_, param_dictionary);
  get_yaml_diag("P0", filename, P(), param_dictionary);
  P0_yaw_ = P()(ErrorState::DQ + 2, ErrorState::DQ + 2);
  get_yaml_diag("R_zero_vel", filename, R_zero_vel_, param_dictionary);

  // Partial Update
  get_yaml_eigen("lambda", filename, lambda_vec_, param_dictionary);
  ...
  // Measurement Flags
  get_yaml_node("enable_partial_update", filename, enable_partial_update_, param_dictionary);
  get_yaml_node("enable_out_of_order", filename, enable_out_of_order_, param_dictionary);
  get_yaml_node("use_mocap", filename, use_mocap_, param_dictionary);
  get_yaml_node("use_gnss", filename, use_gnss_, param_dictionary);
  get_yaml_node("use_baro", filename, use_baro_, param_dictionary);
  get_yaml_node("use_range", filename, use_range_, param_dictionary);
  get_yaml_node("use_zero_vel", filename, use_zero_vel_, param_dictionary);

  // Armed Check
  get_yaml_node("enable_arm_check", filename, enable_arm_check_, param_dictionary);
  get_yaml_node("is_flying_threshold", filename, is_flying_threshold_, param_dictionary);

  // load initial state
  double ref_heading;
  get_yaml_node("ref_heading", filename, ref_heading, param_dictionary);
  q_n2I_ = quat::Quatd::from_euler(0, 0, M_PI/180.0 * ref_heading);

  ref_lla_set_ = false;
  bool manual_ref_lla;
  get_yaml_node("manual_ref_lla", filename, manual_ref_lla, param_dictionary);
  if (manual_ref_lla)
  {
    Vector3d ref_lla;
    get_yaml_eigen("ref_lla", filename, ref_lla, param_dictionary);
  ...
  update_baro_ = false;
  get_yaml_node("update_baro_velocity_threshold", filename, update_baro_vel_thresh_, param_dictionary);

  get_yaml_eigen("x0", filename, x0_.arr(), param_dictionary);

  initLog(filename,param_dictionary);
}

void EKF::initLog(const std::string &filename, const std::string &param_dictionary)
{
  get_yaml_node("enable_log", filename, enable_log_, param_dictionary);
  get_yaml_node("log_prefix", filename, log_prefix_, param_dictionary);
